...
namespace ConstructionToolWithOptions_tf
{
   ...
   ImageSource IEditingCreateToolControl.ActiveTemplateSelectorIcon => null;
        ...
        internal const string BufferOptionNameRatio = "BufferRatio";
        internal const double DefaultBufferRatio = 50;
        ...
        {
            get { return _bufferRatio; }
            set
            {
                if (SetProperty(ref _bufferRatio, value))
                {
                    _isDirty = true;
                    _isValid = true;
                    // add/update the buffer value to the tool options
                    if (!ToolOptions.ContainsKey(BufferOptionNameRatio))
                        ToolOptions.Add(BufferOptionNameRatio, value);
                    else
                        ToolOptions[BufferOptionNameRatio] = value;
                    // ensure options are notified
                    NotifyPropertyChanged(BufferOptionNameRatio);
                }
            }
